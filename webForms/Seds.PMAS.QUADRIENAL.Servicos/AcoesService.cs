...
namespace Seds.PMAS.QUADRIENAL.Servicos
{
    ...
    public class AcoesService : IAcoesService
    {
        ...
        [OperationBehavior(TransactionScopeRequired = true)]
        public Int32 AddAcaoPlanejamento(PrefeituraAcaoPlanejamentoInfo acao)
        {
            ContextManager.OpenConnection();
            try
            {
        ...
        [PrincipalPermission(SecurityAction.Demand, Role = "PMAS QUADRIENAL")]
        [OperationBehavior(TransactionScopeRequired = true)]
        public IList<RecursosPrefeituraAcaoPlanejamentoInfo> GetRecursoAcaoPlanejamentoById(Int32 idPrefeituraAcaoPlanejamento, Int32 idPrefeitura)
        {
            ContextManager.OpenConnection();
            var obj = new PrefeituraAcaoPlanejamento().GetRecursosById(idPrefeituraAcaoPlanejamento,idPrefeitura);
            ContextManager.CloseConnection();
            return obj.ToList();
        }
        ...
        [PrincipalPermission(SecurityAction.Demand, Role = "PMAS QUADRIENAL@Orgão Gestor")]
        [OperationBehavior(TransactionScopeRequired = true)]
        public void SaveRecursoAcaoPlanejamento(RecursosPrefeituraAcaoPlanejamentoInfo acao)
        {
            ContextManager.OpenConnection();
            try
            {
                new PrefeituraAcaoPlanejamento().SaveRecursosPrefeituraAcaoPlanejamentoInfo(acao, true);
                ContextManager.CloseConnection();
            }
            catch (Exception ex)
            {
