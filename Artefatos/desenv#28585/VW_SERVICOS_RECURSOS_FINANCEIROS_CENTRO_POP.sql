...
AS
  ...
  SELECT S.ID
		...
		,S.DETALHAMENTO AS DETALHAMENTO_ENCERRAMENTO
		 ...
		 , ISNULL(S2.EXERCICIO,0) AS EXERCICIO
  
  FROM [dbo].[TB_SERVICOS_RECURSOS_FINANCEIROS_CENTRO_POP] S
  LEFT JOIN TB_SERVICOS_RECURSOS_FINANCEIROS_CENTRO_POP_CAPACIDADE S2 ON S2.ID_SERVICOS_RECURSOS_FINANCEIROS_CENTRO_POP = S.ID
  LEFT JOIN  TB_SERVICOS_RECURSOS_FINANCEIROS_FUNDOS_CENTRO_POP S1 ON(S.ID = S1.ID_SERVICOS_RECURSOS_FINANCEIROS_CENTRO_POP AND S2.EXERCICIO = S1.EXERCICIO)
