...
ALTER VIEW [dbo].[VW_SERVICOS_RECURSOS_FINANCEIROS_PUBLICO]
  ...
  SELECT S.ID
		...
		,CASE WHEN ISNULL((SELECT CAPACIDADE FROM  TB_SERVICOS_RECURSOS_FINANCEIROS_PUBLICO_CAPACIDADE SC WHERE SC.ID_SERVICOS_RECURSOS_FINANCEIROS_PUBLICO = S.ID     AND EXERCICIO = S1.EXERCICIO ),0) != 0 
		       OR ISNULL((SELECT CAPACIDADE FROM   TB_SERVICOS_RECURSOS_FINANCEIROS_PUBLICO_CAPACIDADE_LA  LA WHERE LA.ID_SERVICOS_RECURSOS_FINANCEIROS_PUBLICO = S.ID   AND EXERCICIO = S1.EXERCICIO ),0) != 0 
			   OR ISNULL((SELECT CAPACIDADE FROM   TB_SERVICOS_RECURSOS_FINANCEIROS_PUBLICO_CAPACIDADE_PSC PSC WHERE PSC.ID_SERVICOS_RECURSOS_FINANCEIROS_PUBLICO = S.ID AND EXERCICIO = S1.EXERCICIO ),0) != 0 
		      
			  THEN 
			     ISNULL((SELECT CAPACIDADE FROM  TB_SERVICOS_RECURSOS_FINANCEIROS_PUBLICO_CAPACIDADE SC WHERE SC.ID_SERVICOS_RECURSOS_FINANCEIROS_PUBLICO = S.ID     AND EXERCICIO = S1.EXERCICIO ),0) 
		       + ISNULL((SELECT CAPACIDADE FROM   TB_SERVICOS_RECURSOS_FINANCEIROS_PUBLICO_CAPACIDADE_LA  LA WHERE LA.ID_SERVICOS_RECURSOS_FINANCEIROS_PUBLICO = S.ID   AND EXERCICIO = S1.EXERCICIO ),0)  
			   + ISNULL((SELECT CAPACIDADE FROM   TB_SERVICOS_RECURSOS_FINANCEIROS_PUBLICO_CAPACIDADE_PSC PSC WHERE PSC.ID_SERVICOS_RECURSOS_FINANCEIROS_PUBLICO = S.ID AND EXERCICIO = S1.EXERCICIO ),0)  
			  ELSE 0 END AS PREVISAO_MENSAL_NUMERO_ATENDIDOS
  ...
  LEFT JOIN TB_SERVICOS_RECURSOS_FINANCEIROS_FUNDOS_PUBLICO S1 ON(S.ID = S1.ID_SERVICOS_RECURSOS_FINANCEIROS_PUBLICO)
